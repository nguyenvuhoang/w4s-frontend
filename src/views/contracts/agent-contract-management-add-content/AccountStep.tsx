...
import { ACCOUNT_STATUS_LABELS, ACCOUNT_TABLE_FORMAT, ACCOUNT_TYPE_LABELS } from '@/data/meta';
...
const AccountStep = ({ dictionary, onAccountApplied }: PageContentProps & { onAccountApplied?: () => void }) => {
  const {
    getValues,
    setValue
  } = useFormContext();
  ...
  const accountTypeMap: Record<string, string> = { S: 'DD', T: 'FD' };
  ...
  useEffect(() => {
    ...
    if (account.length > 0) {
      setLeftItems(account.map((row: any, index: number) => ({ id: index, ...row })));
    }
    else
    {
      onAccountApplied?.();
  ...
  const columns = ACCOUNT_TABLE_FORMAT.map((column: any) => {
    ...
    if (column.field === 'accounttype') {
      column.renderCell = (params: any) => ACCOUNT_TYPE_LABELS[params.row.accounttype] || params.row.accounttype;
    }

    if (column.field === 'status') {
      column.renderCell = (params: any) => ACCOUNT_STATUS_LABELS[params.row.status] || params.row.accountstatus;
    }
  ...
  const handleOneClick = () => {
    const selected = leftSelectedRows.map((id: number) => leftItems.find(item => item.id === id));
    const filtered = selected.filter(item => item && !rightItems.some(r => r.accountnumber === item.accountnumber));

    setRightItems(prev => [...prev, ...filtered]);

    setLeftItems(prev => prev.filter(item => !leftSelectedRows.includes(item.id)));
    setLeftSelectedRows([]);
  };

  const handleAllClick = () => {
    const filtered = leftItems.filter(item => !rightItems.some(r => r.accountnumber === item.accountnumber));
  ...
  const handleOneReturnClick = () => {
    const selected = rightSelectedRows.map((id: number) => rightItems.find(item => item.id === id));
    const filtered = selected.filter(item => item && !leftItems.some(l => l.accountnumber === item.accountnumber));

    setLeftItems(prev => [...prev, ...filtered]);

    setRightItems(prev => prev.filter(item => !rightSelectedRows.includes(item.id)));
    setRightSelectedRows([]);
  };

  const handleAllReturnClick = () => {
    const filtered = rightItems.filter(item => !leftItems.some(l => l.accountnumber === item.accountnumber));
  ...
  const handleApplyClick = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const transformedData = rightItems?.map((item: { accountnumber: any; isprimary: string; currency: any; accounttype: any; status: any; branchid: any; bankaccounttype: any; accountname: any }) => ({
          accountnumber: item.accountnumber,
          isprimary: item.isprimary === 'false' ? "0" : "1",
          currencycode: item.currency,
          accounttype: accountTypeMap[item.accounttype] || 'LN',
          status: item.status,
          branchid: item.branchid,
          bankaccounttype: accountTypeMap[item.bankaccounttype] || 'LN',
      ...
      <Grid size={{ xs: 12 }} sx={{ marginTop: '16px' }}>
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid size={{ xs: 2 }} container justifyContent="center" alignItems="center" direction="column" gap={2}>
            {/* Nút thêm từng dòng */}
            <Button variant="outlined" onClick={handleOneClick} disabled={isSubmitted}>
              →
            </Button>

            {/* Nút thêm tất cả */}
            <Button variant="outlined" onClick={handleAllClick} disabled={isSubmitted}>
              ⇒
            </Button>
            {/* Nút bỏ từng dòng */}
            <Button variant="outlined" onClick={handleOneReturnClick} disabled={isSubmitted}>
              ←
            </Button>

            {/* Nút bỏ tất cả */}
            <Button variant="outlined" onClick={handleAllReturnClick} disabled={isSubmitted}>
              ⇐
            </Button>
