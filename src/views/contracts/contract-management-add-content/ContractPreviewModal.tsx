...
const ContractPreviewModal = ({ open, onClose, onConfirm, data, dictionary, locale }: Props) => {
    ...
    const fdAccounts = data.account?.filter(
        (acc: { accounttype: string; bankaccounttype: string }) => acc.accounttype === 'T' || acc.bankaccounttype === 'FD'
    ) || [];
                    ...
                    <Table size="small" sx={{ fontSize: 13 }}>
                        <TableHead>
                            <TableRow>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>#</TableCell>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>{dictionary['account'].accountnumber}</TableCell>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>{dictionary['transaction'].currency}</TableCell>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>
                                    {dictionary['contract']?.tenor }
                                </TableCell>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>
                                    {dictionary['contract']?.opendate || 'Open date'}
                                </TableCell>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>
                                    {dictionary['contract']?.maturitydate || 'Maturity date'}
                                </TableCell>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>
                                    {dictionary['contract']?.account || 'Interest rate'}
                                </TableCell>
                                <TableCell sx={{ border: '1px solid #ccc', fontWeight: 'bold' }}>{dictionary['account'].accountstatus}</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {accounts.map((acc, idx) => (
                                <TableRow key={idx}>
                                    ...
                                    <TableCell sx={{ border: '1px solid #ccc' }}>{acc.accountnumber}</TableCell>
                                    <TableCell sx={{ border: '1px solid #ccc' }}>{acc.currency}</TableCell>
                                    <TableCell sx={{ border: '1px solid #ccc' }}>{acc.tenor || '-'}</TableCell>
                                    <TableCell sx={{ border: '1px solid #ccc' }}>
                                        {acc.opendate ? renderFormattedDate(acc.opendate, locale) : '-'}
                                    </TableCell>
                                    <TableCell sx={{ border: '1px solid #ccc' }}>
                                        {acc.maturitydate ? renderFormattedDate(acc.maturitydate, locale) : '-'}
                                    </TableCell>
                                    <TableCell sx={{ border: '1px solid #ccc' }}>
            ...
            <DialogContent dividers>
                <Box sx={{ fontSize: 13, lineHeight: 1.5 }}>
                    ...
                    <Box sx={{ mt: 4 }}>
                        ...
                        {fdAccounts.length > 0 && renderFDAccountTable(dictionary['contract'].fdaccount, fdAccounts, dictionary, locale)}
