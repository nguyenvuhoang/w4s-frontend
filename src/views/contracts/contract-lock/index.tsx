...
async function apiFetchContractByNo(contractNo: string, session: Session | null): Promise<ContractInfo | null> {
    ...
    const mapped: ContractInfo = {
        ...
        status: safe(first.status),
        statusCaption: safe(first.statuscaption),
...
/** Đổi sang BLOCK */
async function apiBlockContract(
    contractNo: string,
    ...
    try {
        ...
        if (row) {
            ...
            message = norm(msg) || (ok ? 'Block done' : 'Block failed')
        } else {
            ok = true
        }
        ...
        console.error('[BLOCKCONTRACT] error', err)
...
const ContractManagementBlockContent = ({ dictionary, session, locale }: PageContentProps) => {
    ...
    const handleBlock = useCallback(async () => {
        ...
        const result = await Swal.fire({
            text: (dictionary['contract'].block_confirm_text || 'Block contract {0}?')
        ...
        try {
            setIsBlocking(true)
            const res = await apiBlockContract(data.contractNo, session)
            if (res.ok) {
                await Swal.fire({
                    icon: 'success',
                    title: dictionary['contract'].block_success ?? 'Blocked successfully',
                    text: res.message || dictionary['contract'].block_success_desc || 'The contract has been blocked.',
                    confirmButtonText: dictionary['common'].confirm
                })
                // cập nhật local status sang BLOCKED
                setData(prev => (prev ? { ...prev, status: 'B', statusCaption: 'Blocked' } : prev))
            } else {
                await Swal.fire({
                    icon: 'error',
                    title: dictionary['common']?.error,
                    text: res.message || dictionary['contract'].block_error || 'Unable to block this contract.',
    ...
    return (
        <ContentWrapper
            title={`${dictionary['contract'].title} - ${dictionary['common'].block ?? 'Block'}`}
            description={dictionary['contract'].description}
            icon={<LockOutlinedIcon sx={{ fontSize: 40, color: '#dc2626' }} />}
                                    ...
                                    action={
                                        <Button
                                            ...
                                            sx={{
                                                ...
                                                textTransform: 'none',
                                                px: 2,
                                                backgroundColor: canBlock ? '#dc2626' : undefined, // đỏ khi có thể block
                                                '&:disabled': { opacity: 0.5 }
                                            }}
                                        >
                                            {dictionary['common']?.block ?? 'Block'}
                                        </Button>
                                    }
                                />
                                <Divider />
...
export default ContractManagementBlockContent
