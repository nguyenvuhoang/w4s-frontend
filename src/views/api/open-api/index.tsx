...
export default function OpenAPIManagementContent({ dictionary, openAPIdata, session, locale }: PageProps) {
    ...
    const selectedRow =
        openapi?.items.find((x) => x.id === selectedId) || null;


    const isDeleted = (s?: boolean) => s === true
    const isActive = (s?: boolean | string | number) =>
        s === true || s === 1 || s === 'A' || s === 'Y' || s === 'ACTIVE';
    const isProcessing = (s?: boolean) => !isDeleted(s) && !isActive(s)
    const isModify = (s?: boolean) => s === true

    const canDelete = !!selectedRow && isActive(selectedRow.isactive)

    const hasSelection = selected.length > 0

    // Define table columns for dynamic colSpan calculation
    const tableColumns = [
        dict.client_id,
        dict.display_name,
        dict.environment,
    ...
    const totalColumns = tableColumns.length + 1 // +1 for checkbox column

    const handleRowDblClick = (id: string) => {
        if (hasSelection) return;
        const url = getLocalizedUrl(`/coreapi/open-api/view/${id}`, locale as Locale)
                        ...
                        <Grid size={{ xs: 12 }} display="flex" justifyContent="space-between" sx={{ mt: 3 }}>
                            <Box display="flex" gap={2}>
                                ...
                                <Button
                                    ...
                                    onClick={() => selectedRow && window.open(getLocalizedUrl(`/coreapi/open-api/view/${selectedRow.id}`, locale as Locale), '_blank')}
                                >
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        if (selectedRow) {
                                            const url = getLocalizedUrl(`/coreapi/open-api/modify/${selectedRow.id}`, locale as Locale)
                                            window.open(url, '_blank')
                                        }
                                    }}
                                >
                    ...
                    >
                        <TableHead>
                            <TableRow>
                                ...
                                {tableColumns.map((key: string, i: number) => (
                        ...
                        <TableBody>
                            ...
                            ) : !openapi || openapi.items.length === 0 || openapi.items.every(item => Object.keys(item).length === 0) ? (
                                <TableRow>
