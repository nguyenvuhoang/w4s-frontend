...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Check if it's our banking spec simulation
  if (draft.extractedJson?.title === "LMPS Banking Payment Integration (RTGS/Interbank)") {
    draft.openApiSpec = `openapi: 3.0.3
info:
...
paths:
  ...
  /webhooks/payments/status:
    ...
    // Fallback for non-banking simulation
    const openApi = {
      openapi: '3.0.0',
      info: {
        title: draft.extractedJson?.title || draft.title,
        version: '1.0.0'
      },
      // ... (rest of simple generation logic)
      paths: {}
    };
    // Reuse previous logic if needed, but for now we focus on the simulated banking flow
    if (draft.extractedJson?.endpoints) {
      // @ts-ignore
      draft.extractedJson.endpoints.forEach((ep: any) => {
        // @ts-ignore
    ...
    draft.openApiSpec = JSON.stringify(openApi, null, 2);
  }

  return NextResponse.json({ openApiSpec: draft.openApiSpec });
}
