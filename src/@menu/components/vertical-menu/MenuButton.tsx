...
export const menuButtonStyles = (props: MenuButtonStylesProps) => {
  ...
  return css({
    ...
    paddingInlineStart: `${level === 0 ? 20 : (isPopoutWhenCollapsed && isCollapsed ? level : level + 1) * 30}px`,
...
const MenuButton: ForwardRefRenderFunction<HTMLAnchorElement, MenuButtonProps> = (
  ...
  if (component) {
    ...
    if (typeof component === 'string') {
      ...
      const { className: classNameProp, ...props } = (component.props as { className?: string; [key: string]: any })
      ...
      return cloneElement(
        component,
        {
          ...props,
          ...rest,
          ...(className || classNameProp ? { className: classnames(className, classNameProp) } : {}),
          // Do not spread ref here, use React.forwardRef at the export
        },
        children
      )
    }
