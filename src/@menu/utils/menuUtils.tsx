...
export const confirmUrlInChildren = (children: ChildrenType['children'], url: string): boolean => {
  ...
  if (isValidElement(children)) {
    // Type assertion to ensure props is an object with the expected properties
    ...
    const { component, href, exactMatch, activeUrl, children: subChildren } = props

    if (component && component.props && component.props.href) {
      return exactMatch === true || exactMatch === undefined
        ? component.props.href === url
        : typeof activeUrl === 'string' && url.includes(activeUrl)
    }
...
const processMenuChildren = (children: ReactNode, mapFunction: (child: ReactNode) => ReactNode): ReactNode => {
  return Children.map(children, child => {
    ...
    return (child.props as { menuData?: any })?.menuData ? (
      <GenerateVerticalMenu menuData={(child.props as { menuData: any }).menuData} />
    ) : mapFunction(child)
  })
}
...
export const mapHorizontalToVerticalMenu = (children: ReactNode): ReactNode => {
  return Children.map(children, child => {
    ...
    // Explicitly type child as ReactElement<any> to access props safely
    const { children: childChildren, verticalMenuProps, ...rest } = (child as React.ReactElement<any>).props
