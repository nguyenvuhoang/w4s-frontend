...
const PreviewInfo: React.FC<PreviewContentProps> = ({ input, content, onClose, dictionary, language, session }) => {
    ...
    useEffect(() => {
        console.log("ðŸš€ Checking validation...");
        ...
        previewinfo.forEach((section: any) => {
            ...
            try {
                ...
                console.error("âŒ Invalid JSON format in previewinfo:", error);
                sectionData = [];
            }
            ...
            if (section.istable) {
                ...
                parentArray.forEach((rowData: any, rowIndex: number) => {
                    sectionData.forEach((row: any) => {
                        ...
                        console.log(`ðŸ§ Row ${rowIndex} | Field: ${row.field} | Value: ${value} | Required: ${isRequired} | hasError: ${hasError}`);
                ...
                sectionData.forEach((row: any) => {
                    ...
                    console.log(`ðŸ§ Field: ${row.field} | Value: ${value} | Required: ${isRequired} | hasError: ${hasError}`);
        ...
        console.log("âœ… Final validation state:", allValid);
        setIsValid(allValid);
    }, [content]);
    ...
    const handleConfirm = async () => {
        if (!isValid) {
            console.warn("âš ï¸ Please fill all required fields before confirming.");
            return;
        }
        ...
        console.log("âœ… Submitting data to server:", content);
            ...
            {previewinfo.length === 0 ? (
                <Box>
                    <Typography sx={{ textAlign: "center", color: "red", mt: 2, p: 5 }}>
                        âš ï¸ {dictionary['common'].nopreviewinfoconfig}
                    </Typography>
                </Box>
                ...
                <>
                    ...
                    {!isValid && (
                        <Typography sx={{ textAlign: "center", color: "red", mt: 2 }}>
                            âš ï¸ {dictionary['common'].fillrequiredfields}
                        </Typography>
                    )}
